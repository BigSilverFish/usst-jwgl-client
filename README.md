...
### 🎨 现代化沉浸式 UI 与毛玻璃美学
- **全屏沉浸与手势适配**：
  - 采用 Edge-to-Edge 视觉设计，全透明沉浸式状态栏与全面屏手势条无缝融合。
  - 自动适配深色（Dark Mode）与浅色（Light Mode）模式，深浅状态栏文字图标自动反色，彻底告别传统沉重色块。
- **顶栏与底栏一体化磨砂毛玻璃（Frosted Glass）**：
  - 顶部导航栏与底部操作栏统一采用双向磨砂毛玻璃效果与渐变透光设计。
  - 模糊效果严格限制在轮廓边线内生效，向内平滑增加模糊度，实现无缝视觉过渡；课表初始状态下顶栏卡片保持通透清晰，向上划动时呈现优雅的磨砂层次折射。
  - 顶栏底部与底栏顶部均点缀精致微光边线，深色模式与浅色模式下背景色平滑无断层。
- **成绩查询页一体化连贯划动**：
  - 成绩查询页将“学业成绩总览”综合统计卡片与各科详细成绩列表合二为一，支持整体连贯平滑划动，不再冻结置顶挤占屏幕空间。
  - 列表末端精确收窄底部留白，恰如其分地融入精致专属标识。
- **紧凑底栏与人体工学避让**：
  - 底部导航栏经过多轮精细推敲收窄，合理优化文字与图标间距，彻底消除文字截断，同时精准避让全面屏底部手势横条。

### 📅 智能课表系统
- **时刻表双轨支持**：
  - 自动识别新老学期制度，2025-2026-2 及以往历史学期采用 **12节老时刻表**，2026-2027-1 起采用 **13节新时刻表**。
  - 每日首次启动自动云端拉取学期与放假调休信息，避免因教学安排变动带来的错提醒和漏提醒。
